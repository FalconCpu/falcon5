...
class LongTests {
    ...
    @Test
    fun constArrayBasic() {
        // Test basic Long array creation and access

        val prog = """
            const arr = const Array<Long> [0x1111111111112222L, 0x2222222222223333L, 0x3333333333334444L]
    ...
    @Test
    fun longArrayModification() {
        // Test modifying Long array elements
        val prog = """
            fun main()

                var arr = new Array<Long>[0x0000000000000001L, 0x0000000000000002L, 0x0000000000000003L]
                printHex(arr[0])
                arr[0] = 0xFFFFFFFFFFFFFFFFL
                printHex(arr[0])
                arr[1] = arr[1] lsl 32
                printHex(arr[1])
                arr[2] = arr[0] & arr[2]
    ...
    @Test
    fun longArrayIteration() {
        // Test iterating over Long arrays
        val prog = """
            fun main()
                val arr = new Array<Long>[0x1000000000000000L, 0x2000000000000000L, 0x3000000000000000L, 0x4000000000000000L]
                for i in 0..3
                    printHex(arr[i])
        """.trimIndent()
    ...
    @Test
    fun longArrayOperations() {
        // Test performing operations on array elements
        val prog = """
            fun main()
                val arr = new Array<Long>[0x00000000FFFFFFFFL, 0xFFFFFFFF00000000L, 0x0F0F0F0F0F0F0F0FL]
                printHex(arr[0] | arr[1])
                printHex(arr[0] & arr[2])
                printHex(arr[1] lsr 32)
                printHex(arr[0] + arr[1])
        """.trimIndent()
    ...
    @Test
    fun longArrayWithComputation() {
        // Test building array with computed values
        val prog = """
            fun main()
                var arr = new Array<Long>[0L,0L,0L,0L,0L]
                var val_:Long = 1L
                for i in 0..4
                    arr[i] = val_
                    val_ = val_ lsl 8
    ...
    fun longArraySum() {
        ...
        val prog = """
            fun main()
                val arr = new Array<Long>[0x1111111111111111L, 0x2222222222222222L, 0x3333333333333333L, 0x4444444444444444L]
                var sum:Long = 0L
                for i in 0..3
                    sum = sum + arr[i]
                printHex(sum)
    ...
    @Test
    fun longArrayWithCasts() {
        // Test Long arrays with casting from Int
        val prog = """
            fun main()
                var arr = new Array<Long> [0L, 0L, 0L]
                arr[0] = 0x12345678 as Long
                arr[1] = 0xABCDEF01 as Long
                arr[2] = (-1) as Long
                for i in 0..2
                    printHex(arr[i])
        """.trimIndent()
    ...
    @Test
    fun functionReturningLong() {
        val prog = """
            fun getMagicNumber() -> Long
                return 0xDEADBEEFDEADF00DL
            
            fun main()
                val magic:Long = getMagicNumber()
                printHex(magic)
        """.trimIndent()
...
//    @Test
//    fun longArrayIndexExpression() {
//        // Test using expressions as array indices
//        val prog = """
//            fun main()
//                val arr = new Array<Long>[0x1L, 0x2L, 0x4L, 0x8L, 0x10L]
//                val idx:Int = 2
//                printHex(arr[idx])
//                printHex(arr[idx + 1])
//                printHex(arr[0] lsl (arr[3] as Int))
//        """.trimIndent()
//
//        val expected = """
//            0000000000000004
...
//    }
    ...
    fun longInStruct() {
        val prog = """
            struct S(a:Long, b:Int)
            
            fun main()
                val s = S(0x1234567890ABCDEFL, 42)
                printHex(s.a)
                printHex(s.b)
        """.trimIndent()
    ...
    fun polyGlotHash() {
        val prog = """
            ...
            const polyGlotRandom = const Array<Long> [
