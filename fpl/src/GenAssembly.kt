...
private fun Instr.genAssembly() = when(this) {
    ...
    is InstrStore -> "${storeOp(size)} $src, $addr[$offset] # store to address with offset $offset"
    is InstrLoadField -> "${loadOp(size)} $dest, $addr[${offset.offset + offset2}]"
    is InstrStoreField -> "${storeOp(size)} $src, $addr[${offset.offset + offset2}] # store to field ${offset.name} ${offset2}"
